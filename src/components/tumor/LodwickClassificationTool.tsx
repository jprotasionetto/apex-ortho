...
import { lodwickClassificationData } from '@/data/tumor/lodwick-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function LodwickClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? lodwickClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {lodwickClassificationData.grades.map((g) => (
      ...
      {lodwickClassificationData.clinicalPearl && <Alert type="info">{lodwickClassificationData.clinicalPearl}</Alert>}
      {lodwickClassificationData.pitfall && <Alert type="caution">{lodwickClassificationData.pitfall}</Alert>}
    </div>
  );
}
