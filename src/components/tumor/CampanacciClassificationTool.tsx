...
import { campanacciClassificationData } from '@/data/tumor/campanacci-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function CampanacciClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? campanacciClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {campanacciClassificationData.grades.map((g) => (
      ...
      {campanacciClassificationData.clinicalPearl && <Alert type="info">{campanacciClassificationData.clinicalPearl}</Alert>}
      {campanacciClassificationData.pitfall && <Alert type="caution">{campanacciClassificationData.pitfall}</Alert>}
    </div>
  );
}
