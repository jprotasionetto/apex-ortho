...
import { pipkinClassificationData } from '@/data/hip/pipkin-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function PipkinClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? pipkinClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {pipkinClassificationData.grades.map((g) => (
      ...
      {pipkinClassificationData.clinicalPearl && <Alert type="info">{pipkinClassificationData.clinicalPearl}</Alert>}
      {pipkinClassificationData.pitfall && <Alert type="caution">{pipkinClassificationData.pitfall}</Alert>}
    </div>
  );
}
