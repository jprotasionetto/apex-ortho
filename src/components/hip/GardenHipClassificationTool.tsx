...
export default function GardenHipClassificationTool() {
  ...
  const active = selected ? gardenHipClassificationData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {gardenHipClassificationData.clinicalPearl && <Alert type="info">{gardenHipClassificationData.clinicalPearl}</Alert>}
      {gardenHipClassificationData.pitfall && <Alert type="caution">{gardenHipClassificationData.pitfall}</Alert>}
    </div>
  );
}
