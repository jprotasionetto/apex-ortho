...
import { croweClassificationData } from '@/data/hip/crowe-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function CroweClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? croweClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {croweClassificationData.grades.map((g) => (
      ...
      {croweClassificationData.clinicalPearl && <Alert type="info">{croweClassificationData.clinicalPearl}</Alert>}
      {croweClassificationData.pitfall && <Alert type="caution">{croweClassificationData.pitfall}</Alert>}
    </div>
  );
}
