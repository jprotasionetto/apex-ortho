...
import { leggCalvePerthesClassificationData } from '@/data/hip/legg-calve-perthes-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function LeggCalvePerthesClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? leggCalvePerthesClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {leggCalvePerthesClassificationData.grades.map((g) => (
      ...
      {leggCalvePerthesClassificationData.clinicalPearl && <Alert type="info">{leggCalvePerthesClassificationData.clinicalPearl}</Alert>}
      {leggCalvePerthesClassificationData.pitfall && <Alert type="caution">{leggCalvePerthesClassificationData.pitfall}</Alert>}
    </div>
  );
}
