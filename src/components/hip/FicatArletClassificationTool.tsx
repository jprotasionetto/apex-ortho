...
import { ficatArletClassificationData } from '@/data/hip/ficat-arlet-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function FicatArletClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? ficatArletClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {ficatArletClassificationData.grades.map((g) => (
      ...
      {ficatArletClassificationData.clinicalPearl && <Alert type="info">{ficatArletClassificationData.clinicalPearl}</Alert>}
      {ficatArletClassificationData.pitfall && <Alert type="caution">{ficatArletClassificationData.pitfall}</Alert>}
    </div>
  );
}
