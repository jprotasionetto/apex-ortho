...
import { brookerClassificationData } from '@/data/hip/brooker-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function BrookerClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? brookerClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {brookerClassificationData.grades.map((g) => (
      ...
      {brookerClassificationData.clinicalPearl && <Alert type="info">{brookerClassificationData.clinicalPearl}</Alert>}
      {brookerClassificationData.pitfall && <Alert type="caution">{brookerClassificationData.pitfall}</Alert>}
    </div>
  );
}
