...
import { evansClassificationData } from '@/data/hip/evans-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function EvansClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? evansClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {evansClassificationData.grades.map((g) => (
      ...
      {evansClassificationData.clinicalPearl && <Alert type="info">{evansClassificationData.clinicalPearl}</Alert>}
      {evansClassificationData.pitfall && <Alert type="caution">{evansClassificationData.pitfall}</Alert>}
    </div>
  );
}
