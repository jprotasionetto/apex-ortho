...
import { thompsonEpsteinClassificationData } from '@/data/hip/thompson-epstein-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function ThompsonEpsteinClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? thompsonEpsteinClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {thompsonEpsteinClassificationData.grades.map((g) => (
      ...
      {thompsonEpsteinClassificationData.clinicalPearl && <Alert type="info">{thompsonEpsteinClassificationData.clinicalPearl}</Alert>}
      {thompsonEpsteinClassificationData.pitfall && <Alert type="caution">{thompsonEpsteinClassificationData.pitfall}</Alert>}
    </div>
  );
}
