...
import { tonnisClassificationData } from '@/data/hip/tonnis-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function TonnisClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? tonnisClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {tonnisClassificationData.grades.map((g) => (
      ...
      {tonnisClassificationData.clinicalPearl && <Alert type="info">{tonnisClassificationData.clinicalPearl}</Alert>}
      {tonnisClassificationData.pitfall && <Alert type="caution">{tonnisClassificationData.pitfall}</Alert>}
    </div>
  );
}
