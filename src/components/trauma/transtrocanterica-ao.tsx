...
import { transtrocantericaAOData } from '@/data/trauma/transtrocanterica-ao.ts';
import { Alert } from '@/components/ui/Alert.tsx';

const colorMap: Record<string, string> = { safe: '#22C55E', caution: '#F59E0B', danger: '#EF4444' };

const images = [
  { src: '/images/manual-trauma/page161_img1.png', caption: 'AO 31-A — Classificação das fraturas transtrocantéricas: A1 (estável), A2 (instável), A3 (oblíqua reversa)' },
  { src: '/images/manual-trauma/page162_img1.png', caption: 'TAD (Tip-Apex Distance) — medida de Baumgaertner para posicionamento do parafuso cefálico' },
];

export default function TranstrocantericaAOTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const data = transtrocantericaAOData;
  ...
  return (
    <div className="space-y-6">
      <div className="bg-[#111111] border border-[rgba(212,175,55,0.2)] rounded-xl p-4">
        <p className="text-xs text-[#D4AF37] font-medium uppercase tracking-wider mb-3">Fraturas Transtrocantéricas — Manual SBOT (pp. 161–162)</p>
