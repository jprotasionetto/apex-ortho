...
import { patelaAOData } from '@/data/trauma/patela-ao.ts';
import { Alert } from '@/components/ui/Alert.tsx';

const colorMap: Record<string, string> = { safe: '#22C55E', caution: '#F59E0B', danger: '#EF4444' };

const images = [
  { src: '/images/manual-trauma/page184_img1.png', caption: 'AO 34 — Classificação das fraturas da patela: A (avulsão polar), B (transversa/longitudinal), C (cominutiva)' },
];

export default function PatelaAOTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const data = patelaAOData;
