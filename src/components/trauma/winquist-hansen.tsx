...
import { winquistHansenData } from '@/data/trauma/winquist-hansen.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function WinquistHansenTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? winquistHansenData.grades.find((g) => g.id === selected) : null;
    ...
    <div className="space-y-6">
      <div className="bg-[#111111] border border-[rgba(212,175,55,0.15)] rounded-xl p-4 space-y-2">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">Classificação de Winquist-Hansen — Cominuição Diafisária do Fêmur</p>
            <p className="text-xs text-gray-400 mt-1 leading-relaxed">Classifica a cominuição das fraturas diafisárias do fêmur em 5 graus (0-IV) pela porcentagem de contato cortical perdido. Determina a necessidade de bloqueio estático vs dinâmico na haste intramedular.</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-2">
          <div className="bg-[#0A0A0A] rounded-lg p-2">
            <p className="text-[10px] text-[#D4AF37] uppercase tracking-wider font-semibold mb-1">Indicação</p>
            <p className="text-xs text-gray-300 leading-relaxed">Toda fratura diafisária do fêmur tratada com haste intramedular. Avalia a estabilidade da montagem haste-osso e guia o tipo de bloqueio.</p>
          </div>
          <div className="bg-[#0A0A0A] rounded-lg p-2">
            <p className="text-[10px] text-[#D4AF37] uppercase tracking-wider font-semibold mb-1">Relevância</p>
            <p className="text-xs text-gray-300 leading-relaxed">Grau 0-II ({">"}{" "}50% contato): bloqueio dinâmico pode bastar. Grau III-IV ({"<"}{" "}50% contato): bloqueio estático obrigatório para prevenir encurtamento e rotação.</p>
          </div>
        </div>
      </div>
      <div className="bg-[#111111] border border-[rgba(212,175,55,0.2)] rounded-xl p-4">
        <p className="text-xs text-[#D4AF37] font-medium uppercase tracking-wider mb-3">Manual SBOT — Imagens de Referência</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {[
            { src: '/images/manual-trauma/page174_img1.png', caption: 'Fratura diafisária do fêmur — radiografia AP' },
            { src: '/images/manual-trauma/page174_img2.png', caption: 'Fratura diafisária do fêmur — perfil' },
            { src: '/images/manual-trauma/page177_img1.png', caption: 'Consolidação viciosa — fêmur' },
          ].map((img) => (
      ...
      <div className="grid gap-2">
        {winquistHansenData.grades.map((g) => (
      ...
      {winquistHansenData.clinicalPearl && <Alert type="info">{winquistHansenData.clinicalPearl}</Alert>}
      {winquistHansenData.pitfall && <Alert type="caution">{winquistHansenData.pitfall}</Alert>}
    </div>
  );
}
