...
import { meyersMckeeverClassification } from '@/data/knee/meyers-mckeever.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function MeyersMckeeverTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? meyersMckeeverClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {meyersMckeeverClassification.grades.map((g) => (
      ...
      {meyersMckeeverClassification.clinicalPearl && <Alert type="info">{meyersMckeeverClassification.clinicalPearl}</Alert>}
      {meyersMckeeverClassification.pitfall && <Alert type="caution">{meyersMckeeverClassification.pitfall}</Alert>}
    </div>
  );
}
