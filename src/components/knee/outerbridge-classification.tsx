...
export default function OuterbridgeClassificationTool() {
  ...
  return (
    <div className="space-y-6">
      ...
      {outerbridgeClassification.clinicalPearl && <Alert type="info">{outerbridgeClassification.clinicalPearl}</Alert>}
      {outerbridgeClassification.pitfall && <Alert type="caution">{outerbridgeClassification.pitfall}</Alert>}
    </div>
  );
}
