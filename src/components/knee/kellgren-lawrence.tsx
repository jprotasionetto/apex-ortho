...
import { kellgrenLawrenceClassification } from '@/data/knee/kellgren-lawrence.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function KellgrenLawrenceTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? kellgrenLawrenceClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {kellgrenLawrenceClassification.grades.map((g) => (
      ...
      {kellgrenLawrenceClassification.clinicalPearl && <Alert type="info">{kellgrenLawrenceClassification.clinicalPearl}</Alert>}
      {kellgrenLawrenceClassification.pitfall && <Alert type="caution">{kellgrenLawrenceClassification.pitfall}</Alert>}
    </div>
  );
}
