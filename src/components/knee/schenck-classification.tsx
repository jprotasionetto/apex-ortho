...
import { schenckClassification } from '@/data/knee/schenck-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function SchenckClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? schenckClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {schenckClassification.grades.map((g) => (
      ...
      {schenckClassification.clinicalPearl && <Alert type="info">{schenckClassification.clinicalPearl}</Alert>}
      {schenckClassification.pitfall && <Alert type="caution">{schenckClassification.pitfall}</Alert>}
    </div>
  );
}
