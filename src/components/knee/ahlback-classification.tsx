...
import { ahlbackClassification } from '@/data/knee/ahlback-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function AhlbackClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? ahlbackClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {ahlbackClassification.grades.map((g) => (
      ...
      {ahlbackClassification.clinicalPearl && <Alert type="info">{ahlbackClassification.clinicalPearl}</Alert>}
      {ahlbackClassification.pitfall && <Alert type="caution">{ahlbackClassification.pitfall}</Alert>}
    </div>
  );
}
