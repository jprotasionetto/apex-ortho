...
import { dejourTrochlearDysplasiaData } from '@/data/knee/dejour-trochlear-dysplasia.ts';
...
export default function DejourTrochlearDysplasiaTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected
    ? dejourTrochlearDysplasiaData.grades.find((g) => g.id === selected)
    : null;

  return (
    <div className="space-y-6">
      {/* Epidemiology / context block */}
      {dejourTrochlearDysplasiaData.epidemiology && (
      ...
      {/* Mechanism */}
      {dejourTrochlearDysplasiaData.mechanism && (
      ...
      <div>
        ...
        <div className="grid gap-2">
          {dejourTrochlearDysplasiaData.grades.map((g) => (
            <button
              key={g.id}
      ...
      {/* Surgical basis */}
      {dejourTrochlearDysplasiaData.surgicalBasis && (
        <div className="bg-[#111111] border border-slate-700 rounded-xl p-5 space-y-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">
            Base Cirurgica
          </p>
      ...
      {dejourTrochlearDysplasiaData.clinicalPearl && (
        <Alert type="info">{dejourTrochlearDysplasiaData.clinicalPearl}</Alert>
      )}
      {dejourTrochlearDysplasiaData.pitfall && (
        <Alert type="caution">{dejourTrochlearDysplasiaData.pitfall}</Alert>
      )}
    </div>
  );
}
