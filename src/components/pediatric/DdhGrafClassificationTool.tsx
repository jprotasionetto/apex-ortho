...
import { ddhGrafClassificationData } from '@/data/pediatric/ddh-graf.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function DdhGrafClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? ddhGrafClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {ddhGrafClassificationData.grades.map((g) => (
      ...
      {ddhGrafClassificationData.clinicalPearl && <Alert type="info">{ddhGrafClassificationData.clinicalPearl}</Alert>}
      {ddhGrafClassificationData.pitfall && <Alert type="caution">{ddhGrafClassificationData.pitfall}</Alert>}
    </div>
  );
}
