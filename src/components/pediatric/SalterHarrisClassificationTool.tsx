...
export default function SalterHarrisClassificationTool() {
  ...
  const active = selected ? salterHarrisClassificationData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {salterHarrisClassificationData.clinicalPearl && <Alert type="info">{salterHarrisClassificationData.clinicalPearl}</Alert>}
      {salterHarrisClassificationData.pitfall && <Alert type="caution">{salterHarrisClassificationData.pitfall}</Alert>}
    </div>
  );
}
