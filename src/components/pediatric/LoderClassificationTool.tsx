...
import { loderClassificationData } from '@/data/pediatric/loder-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function LoderClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? loderClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {loderClassificationData.grades.map((g) => (
      ...
      {loderClassificationData.clinicalPearl && <Alert type="info">{loderClassificationData.clinicalPearl}</Alert>}
      {loderClassificationData.pitfall && <Alert type="caution">{loderClassificationData.pitfall}</Alert>}
    </div>
  );
}
