...
import { scoliosisRisserClassificationData } from '@/data/pediatric/scoliosis-risser.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function ScoliosisRisserClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? scoliosisRisserClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {scoliosisRisserClassificationData.grades.map((g) => (
      ...
      {scoliosisRisserClassificationData.clinicalPearl && <Alert type="info">{scoliosisRisserClassificationData.clinicalPearl}</Alert>}
      {scoliosisRisserClassificationData.pitfall && <Alert type="caution">{scoliosisRisserClassificationData.pitfall}</Alert>}
    </div>
  );
}
