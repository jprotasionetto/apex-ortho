...
import { dimeglioClassificationData } from '@/data/pediatric/dimeglio-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function DimeglioClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? dimeglioClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {dimeglioClassificationData.grades.map((g) => (
      ...
      {dimeglioClassificationData.clinicalPearl && <Alert type="info">{dimeglioClassificationData.clinicalPearl}</Alert>}
      {dimeglioClassificationData.pitfall && <Alert type="caution">{dimeglioClassificationData.pitfall}</Alert>}
    </div>
  );
}
