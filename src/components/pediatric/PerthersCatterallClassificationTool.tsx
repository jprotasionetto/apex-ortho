...
import { perthersCatterallClassificationData } from '@/data/pediatric/perthes-catterall.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function PerthersCatterallClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? perthersCatterallClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {perthersCatterallClassificationData.grades.map((g) => (
      ...
      {perthersCatterallClassificationData.clinicalPearl && <Alert type="info">{perthersCatterallClassificationData.clinicalPearl}</Alert>}
      {perthersCatterallClassificationData.pitfall && <Alert type="caution">{perthersCatterallClassificationData.pitfall}</Alert>}
    </div>
  );
}
