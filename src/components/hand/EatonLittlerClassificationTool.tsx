...
import { eatonLittlerClassificationData } from '@/data/hand/eaton-littler-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function EatonLittlerClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? eatonLittlerClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {eatonLittlerClassificationData.grades.map((g) => (
      ...
      {eatonLittlerClassificationData.clinicalPearl && <Alert type="info">{eatonLittlerClassificationData.clinicalPearl}</Alert>}
      {eatonLittlerClassificationData.pitfall && <Alert type="caution">{eatonLittlerClassificationData.pitfall}</Alert>}
    </div>
  );
}
