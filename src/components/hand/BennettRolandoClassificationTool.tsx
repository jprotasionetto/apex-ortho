...
import { bennettRolandoClassificationData } from '@/data/hand/bennett-rolando-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function BennettRolandoClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? bennettRolandoClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {bennettRolandoClassificationData.grades.map((g) => (
      ...
      {bennettRolandoClassificationData.clinicalPearl && <Alert type="info">{bennettRolandoClassificationData.clinicalPearl}</Alert>}
      {bennettRolandoClassificationData.pitfall && <Alert type="caution">{bennettRolandoClassificationData.pitfall}</Alert>}
    </div>
  );
}
