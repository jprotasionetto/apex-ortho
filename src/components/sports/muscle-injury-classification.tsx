...
import { muscleInjuryClassification } from '@/data/sports/muscle-injury-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function MuscleInjuryClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? muscleInjuryClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {muscleInjuryClassification.grades.map((g) => (
      ...
      {muscleInjuryClassification.clinicalPearl && <Alert type="info">{muscleInjuryClassification.clinicalPearl}</Alert>}
      {muscleInjuryClassification.pitfall && <Alert type="caution">{muscleInjuryClassification.pitfall}</Alert>}
    </div>
  );
}
