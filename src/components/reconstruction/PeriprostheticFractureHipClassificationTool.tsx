...
import { periprostheticFractureHipClassificationData } from '@/data/reconstruction/periprosthetic-fracture-hip.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function PeriprostheticFractureHipClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? periprostheticFractureHipClassificationData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {periprostheticFractureHipClassificationData.clinicalPearl && <Alert type="info">{periprostheticFractureHipClassificationData.clinicalPearl}</Alert>}
      {periprostheticFractureHipClassificationData.pitfall && <Alert type="caution">{periprostheticFractureHipClassificationData.pitfall}</Alert>}
    </div>
  );
}
