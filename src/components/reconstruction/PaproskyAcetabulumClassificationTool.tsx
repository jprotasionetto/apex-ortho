...
import { paproskyAcetabulumClassificationData } from '@/data/reconstruction/paprosky-acetabulum.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function PaproskyAcetabulumClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? paproskyAcetabulumClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {paproskyAcetabulumClassificationData.grades.map((g) => (
      ...
      {paproskyAcetabulumClassificationData.clinicalPearl && <Alert type="info">{paproskyAcetabulumClassificationData.clinicalPearl}</Alert>}
      {paproskyAcetabulumClassificationData.pitfall && <Alert type="caution">{paproskyAcetabulumClassificationData.pitfall}</Alert>}
    </div>
  );
}
