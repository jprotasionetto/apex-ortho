...
import { paproskyfemurClassificationData } from '@/data/reconstruction/paprosky-femur.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function PaproskyFemurClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? paproskyfemurClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {paproskyfemurClassificationData.grades.map((g) => (
      ...
      {paproskyfemurClassificationData.clinicalPearl && <Alert type="info">{paproskyfemurClassificationData.clinicalPearl}</Alert>}
      {paproskyfemurClassificationData.pitfall && <Alert type="caution">{paproskyfemurClassificationData.pitfall}</Alert>}
    </div>
  );
}
