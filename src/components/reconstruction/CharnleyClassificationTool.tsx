...
import { charnleyClassificationData } from '@/data/reconstruction/charnley-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function CharnleyClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? charnleyClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {charnleyClassificationData.grades.map((g) => (
      ...
      {charnleyClassificationData.clinicalPearl && <Alert type="info">{charnleyClassificationData.clinicalPearl}</Alert>}
      {charnleyClassificationData.pitfall && <Alert type="caution">{charnleyClassificationData.pitfall}</Alert>}
    </div>
  );
}
