...
import { andersonDalonzoClassification } from '@/data/spine/anderson-dalonzo.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function AndersonDalonzoTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? andersonDalonzoClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {andersonDalonzoClassification.grades.map((g) => (
      ...
      {andersonDalonzoClassification.clinicalPearl && <Alert type="info">{andersonDalonzoClassification.clinicalPearl}</Alert>}
      {andersonDalonzoClassification.pitfall && <Alert type="caution">{andersonDalonzoClassification.pitfall}</Alert>}
    </div>
  );
}
