...
import { pfirrmannClassification } from '@/data/spine/pfirrmann-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';
import { PfirrmannSVG } from '@/components/svg/spine/PfirrmannSVG.tsx';

export default function PfirrmannClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? pfirrmannClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 overflow-hidden">
        <PfirrmannSVG activeGrade={selected} onGradeSelect={setSelected} interactive={true} showLabels={true} />
      </div>
      <div className="grid gap-2">
        {pfirrmannClassification.grades.map((g) => (
      ...
      {pfirrmannClassification.clinicalPearl && <Alert type="info">{pfirrmannClassification.clinicalPearl}</Alert>}
      {pfirrmannClassification.pitfall && <Alert type="caution">{pfirrmannClassification.pitfall}</Alert>}
    </div>
  );
}
