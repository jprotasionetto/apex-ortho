...
import { denisClassificationSpine } from '@/data/spine/denis-classification-spine.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function DenisClassificationSpineTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? denisClassificationSpine.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      {/* Manual Images */}
      <div className="pt-2 pb-4 border-b border-[rgba(212,175,55,0.15)]">
        <p className="text-xs font-semibold text-[#D4AF37] uppercase tracking-widest mb-3">Manual SBOT — Imagens de Referência (pp. 329–335)</p>
        <div className="grid grid-cols-2 gap-2">
          {[
            { src: '/images/manual-trauma/page329_img1.png', alt: 'Três colunas de Denis — anterior, média, posterior' },
            { src: '/images/manual-trauma/page330_img1.png', alt: 'Fratura por compressão — coluna anterior' },
            { src: '/images/manual-trauma/page331_img1.png', alt: 'Fratura burst — colapso das colunas' },
            { src: '/images/manual-trauma/page332_img1.png', alt: 'Fratura por flexão-distração (Chance)' },
            { src: '/images/manual-trauma/page335_img1.png', alt: 'Fratura-luxação — instabilidade total' },
          ].map((img, idx) => (
      ...
      <div className="grid gap-2">
        {denisClassificationSpine.grades.map((g) => (
      ...
      {denisClassificationSpine.clinicalPearl && <Alert type="info">{denisClassificationSpine.clinicalPearl}</Alert>}
      {denisClassificationSpine.pitfall && <Alert type="caution">{denisClassificationSpine.pitfall}</Alert>}
    </div>
  );
}
