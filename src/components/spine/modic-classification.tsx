...
import { modicClassification } from '@/data/spine/modic-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function ModicClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? modicClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {modicClassification.grades.map((g) => (
      ...
      {modicClassification.clinicalPearl && <Alert type="info">{modicClassification.clinicalPearl}</Alert>}
      {modicClassification.pitfall && <Alert type="caution">{modicClassification.pitfall}</Alert>}
    </div>
  );
}
