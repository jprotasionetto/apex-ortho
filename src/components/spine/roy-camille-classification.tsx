...
import { royCamilleClassificationData } from '@/data/spine/roy-camille-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function RoyCamilleClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? royCamilleClassificationData.grades.find((g) => g.id === selected) : null;
    ...
    <div className="space-y-6">
      <div className="bg-[#111111] border border-[rgba(212,175,55,0.15)] rounded-xl p-4 space-y-2">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">Classificação de Roy-Camille — Fraturas Transversas do Sacro</p>
            <p className="text-xs text-gray-400 mt-1 leading-relaxed">Classifica fraturas transversas do sacro em 3 tipos pelo grau de angulação e deslocamento do fragmento distal. Complementa as Zonas de Denis (fraturas verticais). Frequentemente subdiagnosticadas — solicitar TC sagital.</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 mt-2">
          <div className="bg-[#0A0A0A] rounded-lg p-2">
            <p className="text-[10px] text-[#D4AF37] uppercase tracking-wider font-semibold mb-1">Indicação</p>
            <p className="text-xs text-gray-300 leading-relaxed">Fraturas transversas do sacro (padrão horizontal). Presentes em 30-45% das fraturas pélvicas de alta energia. Déficit neurológico em 25-50%.</p>
          </div>
          <div className="bg-[#0A0A0A] rounded-lg p-2">
            <p className="text-[10px] text-[#D4AF37] uppercase tracking-wider font-semibold mb-1">Relevância</p>
            <p className="text-xs text-gray-300 leading-relaxed">Tipo 1: conservador se intacto neurologicamente. Tipo 2: redução + fixação. Tipo 3: cirurgia urgente (deslocamento completo, déficit em {">"}90%).</p>
          </div>
        </div>
      </div>
      <div className="bg-[#111111] border border-[rgba(212,175,55,0.2)] rounded-xl p-4">
        <p className="text-xs text-[#D4AF37] font-medium uppercase tracking-wider mb-3">Manual SBOT — Pelve e Sacro</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {[
            { src: '/images/manual-trauma/page143_img1.png', caption: 'Epidemiologia — fraturas pélvicas' },
            { src: '/images/manual-trauma/page144_img1.png', caption: 'Anatomia — corona mortis e pelve' },
          ].map((img) => (
      ...
      <div className="grid gap-2">
        {royCamilleClassificationData.grades.map((g) => (
      ...
      {royCamilleClassificationData.clinicalPearl && <Alert type="info">{royCamilleClassificationData.clinicalPearl}</Alert>}
      {royCamilleClassificationData.pitfall && <Alert type="caution">{royCamilleClassificationData.pitfall}</Alert>}
    </div>
  );
}
