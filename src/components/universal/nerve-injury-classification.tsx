...
import { nerveInjuryClassification } from '@/data/universal/nerve-injury-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function NerveInjuryClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? nerveInjuryClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {nerveInjuryClassification.grades.map((g) => (
      ...
      {nerveInjuryClassification.clinicalPearl && <Alert type="info">{nerveInjuryClassification.clinicalPearl}</Alert>}
      {nerveInjuryClassification.pitfall && <Alert type="caution">{nerveInjuryClassification.pitfall}</Alert>}
    </div>
  );
}
