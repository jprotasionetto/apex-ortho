...
import { woundClassification } from '@/data/universal/wound-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function WoundClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? woundClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {woundClassification.grades.map((g) => (
      ...
      {woundClassification.clinicalPearl && <Alert type="info">{woundClassification.clinicalPearl}</Alert>}
      {woundClassification.pitfall && <Alert type="caution">{woundClassification.pitfall}</Alert>}
    </div>
  );
}
