...
import { clavienDindoClassification } from '@/data/universal/clavien-dindo.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function ClavienDindoTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? clavienDindoClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {clavienDindoClassification.grades.map((g) => (
      ...
      {clavienDindoClassification.clinicalPearl && <Alert type="info">{clavienDindoClassification.clinicalPearl}</Alert>}
      {clavienDindoClassification.pitfall && <Alert type="caution">{clavienDindoClassification.pitfall}</Alert>}
    </div>
  );
}
