...
import { asaClassification } from '@/data/universal/asa-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function AsaClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? asaClassification.grades.find((g) => g.id === selected) : null;
  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {asaClassification.grades.map((g) => (
      ...
      {asaClassification.clinicalPearl && <Alert type="info">{asaClassification.clinicalPearl}</Alert>}
      {asaClassification.pitfall && <Alert type="caution">{asaClassification.pitfall}</Alert>}
    </div>
  );
}
