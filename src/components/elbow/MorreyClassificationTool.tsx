...
import { morreyClassificationData } from '@/data/elbow/morrey-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function MorreyClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? morreyClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {morreyClassificationData.grades.map((g) => (
      ...
      {morreyClassificationData.clinicalPearl && <Alert type="info">{morreyClassificationData.clinicalPearl}</Alert>}
      {morreyClassificationData.pitfall && <Alert type="caution">{morreyClassificationData.pitfall}</Alert>}
    </div>
  );
}
