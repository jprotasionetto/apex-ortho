...
import { oconnorClassificationData } from '@/data/elbow/oconnor-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function OconnorClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? oconnorClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {oconnorClassificationData.grades.map((g) => (
      ...
      {oconnorClassificationData.clinicalPearl && <Alert type="info">{oconnorClassificationData.clinicalPearl}</Alert>}
      {oconnorClassificationData.pitfall && <Alert type="caution">{oconnorClassificationData.pitfall}</Alert>}
    </div>
  );
}
