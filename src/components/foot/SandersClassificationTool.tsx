...
import { sandersClassification } from '@/data/foot/sanders-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';
import { SandersSVG } from '@/components/svg/foot/SandersSVG.tsx';

export default function SandersClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? sandersClassification.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 overflow-hidden">
        <SandersSVG activeGrade={selected} onGradeSelect={setSelected} interactive={true} showLabels={true} />
      </div>
      <div className="grid gap-2">
        {sandersClassification.grades.map((g) => (
      ...
      {sandersClassification.clinicalPearl && <Alert type="info">{sandersClassification.clinicalPearl}</Alert>}
      {sandersClassification.pitfall && <Alert type="caution">{sandersClassification.pitfall}</Alert>}
    </div>
  );
}
