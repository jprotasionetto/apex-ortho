...
import { diabeticFootClassification } from '@/data/foot/diabetic-foot-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function DiabeticFootClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? diabeticFootClassification.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {diabeticFootClassification.grades.map((g) => (
      ...
      {diabeticFootClassification.clinicalPearl && <Alert type="info">{diabeticFootClassification.clinicalPearl}</Alert>}
      {diabeticFootClassification.pitfall && <Alert type="caution">{diabeticFootClassification.pitfall}</Alert>}
    </div>
  );
}
