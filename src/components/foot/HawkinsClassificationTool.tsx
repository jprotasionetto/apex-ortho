...
import { hawkinsClassification } from '@/data/foot/hawkins-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';
import { HawkinsSVG } from '@/components/svg/foot/HawkinsSVG.tsx';

export default function HawkinsClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? hawkinsClassification.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 overflow-hidden">
        <HawkinsSVG activeGrade={selected} onGradeSelect={setSelected} interactive={true} showLabels={true} />
      </div>
      <div className="grid gap-2">
        {hawkinsClassification.grades.map((g) => (
      ...
      {hawkinsClassification.clinicalPearl && <Alert type="info">{hawkinsClassification.clinicalPearl}</Alert>}
      {hawkinsClassification.pitfall && <Alert type="caution">{hawkinsClassification.pitfall}</Alert>}
    </div>
  );
}
