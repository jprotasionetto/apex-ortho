...
import { samilsonPrietoData } from '@/data/shoulder/samilson-prieto.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function SamilsonPrietoTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? samilsonPrietoData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {samilsonPrietoData.clinicalPearl && <Alert type="info">{samilsonPrietoData.clinicalPearl}</Alert>}
      {samilsonPrietoData.pitfall && <Alert type="caution">{samilsonPrietoData.pitfall}</Alert>}
    </div>
  );
}
