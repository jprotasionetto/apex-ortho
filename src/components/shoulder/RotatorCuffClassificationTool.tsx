...
import { rotatorCuffClassificationData } from '@/data/shoulder/rotator-cuff-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function RotatorCuffClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? rotatorCuffClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {rotatorCuffClassificationData.grades.map((g) => (
      ...
      {rotatorCuffClassificationData.clinicalPearl && <Alert type="info">{rotatorCuffClassificationData.clinicalPearl}</Alert>}
      {rotatorCuffClassificationData.pitfall && <Alert type="caution">{rotatorCuffClassificationData.pitfall}</Alert>}
    </div>
  );
}
