...
export default function WalchClassificationTool() {
  ...
  const active = selected ? walchClassificationData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      {walchClassificationData.clinicalPearl && <Alert type="info">{walchClassificationData.clinicalPearl}</Alert>}
      {walchClassificationData.pitfall && <Alert type="caution">{walchClassificationData.pitfall}</Alert>}
    </div>
  );
}
