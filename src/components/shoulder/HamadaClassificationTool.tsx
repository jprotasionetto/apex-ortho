...
import { hamadaClassificationData } from '@/data/shoulder/hamada-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';

export default function HamadaClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? hamadaClassificationData.grades.find((g) => g.id === selected) : null;

  return (
    <div className="space-y-6">
      <div className="grid gap-2">
        {hamadaClassificationData.grades.map((g) => (
      ...
      {hamadaClassificationData.clinicalPearl && <Alert type="info">{hamadaClassificationData.clinicalPearl}</Alert>}
      {hamadaClassificationData.pitfall && <Alert type="caution">{hamadaClassificationData.pitfall}</Alert>}
    </div>
  );
}
