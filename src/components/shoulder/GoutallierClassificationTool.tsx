...
import { goutallierClassificationData } from '@/data/shoulder/goutallier-classification.ts';
import { Alert } from '@/components/ui/Alert.tsx';
import GoutallierSVG from '@/components/svg/shoulder/GoutallierSVG.tsx';

export default function GoutallierClassificationTool() {
  const [selected, setSelected] = useState<string | null>(null);
  const active = selected ? goutallierClassificationData.grades.find((g) => g.id === selected) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-2">
        {goutallierClassificationData.grades.map((g) => (
      ...
      {goutallierClassificationData.clinicalPearl && <Alert type="info">{goutallierClassificationData.clinicalPearl}</Alert>}
      {goutallierClassificationData.pitfall && <Alert type="caution">{goutallierClassificationData.pitfall}</Alert>}
    </div>
  );
}
