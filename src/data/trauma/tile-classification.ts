...
export const tileClassificationData: ClassificationData = {
  ...
  author: 'Tile',
  year: 1988,
  epidemiology: 'Fraturas do anel pélvico representam ~3% de todas as fraturas e ~8% das fraturas em politraumatizados. Incidência: ~20/100.000/ano. Mortalidade hospitalar: Tile A <1%, Tile B 5–10%, Tile C 10–20% (hemorragia + lesões associadas em múltiplos órgãos). Distribuição: Tile A ~60%, Tile B ~25%, Tile C ~15%. Mecanismo bimodal: jovens (acidente de trânsito: ~75% das Tile B–C) e idosos (queda de baixa energia: maioria Tile A, mas fraturas sacras insuficiência por osteoporose são subdiagnosticadas). Hemorragia: o retroperitônio pélvico pode comportar 3–5 litros — principal causa de morte nas primeiras 24h. Fontes hemorrágicas: plexo venoso sacral (70–90% dos sangrados) e artérias (10–30%, principalmente pudenda interna + obturatória). Lesões associadas: urológicas 15–20% (uretra em homens B1+C), neurológicas 30% nas Tile C (plexo L4-S3 no sacro Denis zona III), intestinais 5–10%. Fratura de Morel-Lavallée (deglovamento subcutâneo) em 20% das fraturas de alta energia.',
  mechanism: 'Tile (1988) define estabilidade pelo ARCO POSTERIOR (ligamentos sacroilíacos posteriores = a âncora vertical do anel pélvico). Young-Burgess (1986) classifica pelo vetor cinemático — é complementar e mais útil no trauma agudo. Tipo A (arco posterior intacto): energia baixa ou força tangencial → avulsões musculares (EIAS: sartório+tensor; EIAI: reto femoral; tuberosidade isquiática: isquiotibiais); fraturas do ala ilíaca sem extensão para SI. Tipo B (ruptura INCOMPLETA do arco posterior — ligamentos SI posteriores INTACTOS): B1 (open-book) = força AP comprime o anel → abertura em livro → rotação externa do hemipelve → diástase sinfisária + ligamentos SI anteriores rompidos; B2 (lateral compression) = força lateral → rotação interna → impacção anterior do sacro. Tipo C (ruptura COMPLETA — ligamentos SI posteriores ROMPIDOS): cisalhamento vertical ou compressão AP extrema → o hemipelve se solta completamente do axial → instabilidade vertical (ascensão > 1cm) + rotacional + hemorragia massiva do plexo sacral.',
  referenceImages: [
    {
      src: 'https://upload.wikimedia.org/wikipedia/commons/0/04/Pelvic_Fracture_Young-Burgess_Classification.png',
      caption: 'Classificação de Young-Burgess das fraturas pélvicas (complementar à Tile) — diagrama mostrando padrões LC, APC e VS. Fonte: Wikimedia Commons, Domínio Público.',
      source: 'Wikimedia Commons — Domínio Público'
    },
    {
      src: 'https://upload.wikimedia.org/wikipedia/commons/3/3d/Diastasis_symphysis_pubis_1300500.JPG',
      caption: 'Diástase da sínfise púbica — radiografia AP mostrando lesão do arco anterior (padrão Tile B1, open-book). Fonte: Wikimedia Commons.',
