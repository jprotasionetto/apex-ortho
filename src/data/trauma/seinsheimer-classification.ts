...
export const seinshheimerClassificationData: ClassificationData = {
  ...
  epidemiology: 'Fraturas subtrocantéricas representam 10–15% de todas as fraturas do fêmur proximal. Incidência: ~5/100.000/ano. Distribuição bimodal: jovens (alta energia: acidentes e quedas de altura — >60% dos casos em jovens) e idosos (baixa energia, osteoporose — maioria). Mortalidade em 1 ano em idosos: 15–35% (semelhante às intertrocantéricas, mas implante técnicamente mais exigente). Pseudoartrose: 5–15% nas de alta energia (vs <2% nas intertrocantéricas) — reflexo das forças deformantes máximas da região. Fraturas atípicas por bifosfonatos (AFF): incidência crescente — 3–50/100.000 pacientes-ano de exposição; risco aumenta com duração (>10 anos = risco 5×). Bilateral em 28–48% das AFF (RX bilateral obrigatório). Fratura atípica incompleta ("dreaded black line" na cortical lateral) = indicação de cirurgia profilática. Complicações com DHS em obliquidade reversa: falha mecânica em 30–50% — a principal indicação errônea em prova.',
  mechanism: 'A região subtrocantérica (trocânter menor até 5 cm distais) é a zona de maior stress mecânico do fêmur. Forças deformantes após a fratura — fragmento PROXIMAL: (1) Iliopsoas (inserção no trocânter menor) → FLEXÃO; (2) Glúteos médio+mínimo (inserção no trocânter maior) → ABDUÇÃO; (3) Rotadores curtos (piriforme, gêmeos, obturador) → ROTAÇÃO EXTERNA. Fragmento DISTAL: adução + encurtamento pelos quadríceps e isquiotibiais. Esta "posição fixa" em flexão+abdução+RE deve ser CORRIGIDA durante a inserção da haste — o principal erro técnico é não compensar este desvio. Fratura atípica por bifosfonatos (AFF): inibição excessiva da remodelação óssea → acúmulo de microfissuras na cortical lateral (zona de máxima tensão em tração) → fratura de fadiga progressiva → "dreaded black line" → fratura completa transversa com mínimo trauma. DHS convencional na fratura de obliquidade reversa (AO 31A3.1) é CONTRAINDICADO: o padrão de carga transmite cisalhamento ao parafuso → falha mecânica inevitável — usar haste cefalomedular ou placa-lâmina 95°.',
  referenceImages: [
    {
      src: 'https://upload.wikimedia.org/wikipedia/commons/1/11/Cdm_hip_fracture_343.jpg',
      caption: 'Fratura do fêmur proximal — a região subtrocantérica (Seinsheimer) está imediatamente abaixo do trocânter menor, sujeita às maiores forças deformantes do fêmur. Fonte: Wikimedia Commons, CC BY-SA 3.0.',
